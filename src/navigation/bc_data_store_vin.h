...
class DataStore{
    ...
    public: 
        ...
        void construct_input(){
            ...
            auto x = curr_odom[0];
            auto y = curr_odom[1];
            auto angle = atan2(global_goal_[1] - y, global_goal_[0] - x)* 2 / M_PI;
        ...
        void post_processing_costmap(std_msgs::Float32MultiArray msg){
            float angle = msg.data[msg.data.size()-1];
            float odom_time = msg.data[msg.data.size()-2];
            float odom_theta = msg.data[msg.data.size()-3];
            float odom_y = msg.data[msg.data.size()-4];
            float odom_x = msg.data[msg.data.size()-5];
        ...
        Eigen::Vector2f get_bc_target(){
            ...
            int counter = 3;
            while(dt - counter * time_interval_ > 0){
                counter += 1;
            }

            auto mat = build_transform_matrix(curr_odom.data, {0,0,0});

            float x = traj_to_execute_[counter-1].first;
            float y = traj_to_execute_[counter-1].second;
